...
class Misc {
  ...
  createNewToken() {
    const token = {}
    token.timestamp = this.getSystemRunTime()
    token.token = md5("adp" + Math.random() + "" + token.timestamp)
    return token
  }
  //开始检查日志图的记录，保留最近七天的日志图
  initLogImgCheck(last_day) {
    if (this.logCheckTimer != 0) {
      clearTimeout(this.logCheckTimer)
    }
    if (last_day <= 0) {
      fs.rm(conf.cfgDataPath() + "/log/" + fname).then((res) => {
        fs.mkdir(this.cfgDataPath() + "/log")
      })
      this.logCheckTimer = setTimeout(() => {
        this.initLogImgCheck(7)
      }, 60 * 60 * 8 * 1000)
      return
    }
    misc
      .disk_space_percent({ path: conf.cfgDataPath() })
      .then((res) => {
        console.log(res)
        if (res.percent < 10) {
          //只剩10%以内的空间
          this.cleanupLogr(last_day)
        } else if (last_day === 7) {
          //空间够，只有7天的时候才删
          this.cleanupLogr(last_day)
        } else {
  ...
  async cleanupLogr(days) {
    //清除指定天数
    let ts = new Date().getTime()
    const flist = await fs.readdir(conf.cfgDataPath() + "/log")
    for (let fname of flist) {
      let tt = Date.parse(fname + " 00:00:00")
      // console.log(`-------------------------fname ${fname}`)
      // console.log(`-------------------------now ${ts}`)
      // console.log(`-------------------------tt ${tt}`)
      // console.log(
      //   `----------------ts - 60 * 60 * 24 * days : ${ts - 60 * 60 * 24 * days}`
      // )
      if (ts - tt > 60 * 60 * 24 * days * 1000) {
        // console.log(
        //   `--------------------------rm  ${
        //     conf.cfgDataPath() + "/log/" + fname
        //   }`
        // )
        fs.rm(conf.cfgDataPath() + "/log/" + fname)
      }
    }

    this.initLogImgCheck(days - 1)
  }
  //判断用户权限
