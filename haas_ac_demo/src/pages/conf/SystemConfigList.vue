...
<template>
  ...
  >
    <div class="common-wrap">
      <fl-button type="info" @click="clickReboot()"> 设备重启 </fl-button>
    </div>

    <div class="common-wrap">
      <fl-button type="info" @click="clickReset()">{{
        "清空用户和识别数据"
      }}</fl-button>
    </div>
    ...
    <fl-dialog
      ...
      cancelText="取消"
      confirmText="确认"
      :handleConfirmClick="confirmReset"
      content="请确认操作,操作完成后会主动重启"
    >
    </fl-dialog>
...
<script>
import { FlSeekbar, FlButton, FlDialog } from "falcon-ui"
...
export default {
  ...
  components: { FlSeekbar, FlButton, listChoosePop, inputPop, FlDialog },
  data() {
    return {
      configs: [], //当前页面的配置们
      rangeTimer: null,
  ...
  methods: {
    ...
    async confirmReset() {
      ...
      let ret = await fs.rm(conf.cfgDataPath())
      if (ret) {
        misc.reboot()
      } else {
        this.$refs.toast.show("操作发生异常")
      }
    },
    clickReset() {
